...
describe("SessionsService", () => {
  ...
  describe("create", () => {
    it("should create session with tickets", async () => {
      ticketsService.selectForSession.mockResolvedValue([
        "t1",
        "t2",
        "t3",
      ]);
        ...
        tickets: [
          {
            ...
            answer: { selectedOptionId: "o1", isCorrect: true, timeMs: 1000, answeredAt: new Date(), id: "a1" },
          },
          {
            id: "st2",
            order: 2,
  ...
  describe("finish", () => {
    ...
    it("should throw if already finished", async () => {
      ...
      await expect(service.finish("s1")).rejects.toThrow(
        BadRequestException,
      );
    });
